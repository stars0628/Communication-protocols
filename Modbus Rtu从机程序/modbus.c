...
unsigned char
    usDiscreteInputBuf[DISCRETE_INPUT_NDISCRETES / 8]; // 离散输入寄存器
...
static unsigned short MB_CRC16(unsigned char *puchMsg,
                               unsigned short usDataLen);
static unsigned char MB_Parse_Data(unsigned char *data, unsigned short count);
...
/* 提取数据帧,进行解析数据帧 */
static unsigned char MB_Parse_Data(unsigned char *data, unsigned short count) {
  // 数据帧仅包含地址字段和功能码字段，加上CRC校验，总长度至少为4个字节
  if (count < 4) {
    return 1;
  }
  ...
  PduData.Num =
      ((data[4] << 8) | data[5]); // 数量(Coil,Input,Holding Reg,Input Reg)
  ...
  PduData.PtrCoilOffset =
      PduData.PtrCoilbase + PduData.Addr; // 离散量的内存起始地址
  PduData.PtrHoldingOffset =
      PduData.PtrHoldingbase + PduData.Addr; // 保持寄存器的起始地址
  return 0;
}
...
static unsigned char MB_Data_Check(unsigned char *data, unsigned short count) {
  ...
  /* 检查地址 */
  if (PduData.SlaveAddr != MB_SLAVEADDR &&
      PduData.SlaveAddr != MB_ALLSLAVEADDR) {
...
static void MB_Fun1_2(unsigned char *buffer) {
  ...
  if (PduData.Num % 8 ==
      0) // 如果读取线圈的数量是8的整数倍，则返回字节数MB_TxBuf[2]
         // = PduData.Num / 8
...
static void MB_Fun3_4(unsigned short *buffer) {
  ...
  tem_crc =
      MB_CRC16((unsigned char *)MB_TxBuf, PduData.Num * 2 + 3); // CRC校验码
...
unsigned char MB_Analyze_Execute(unsigned char *data, unsigned short count) {
  ...
  case FUN_CODE_10H: // 写入多个保持寄存器
    for (int i = 0; i < PduData.Num; i++) {
      usRegHoldingBuf[PduData.Addr + i] =
          (data[i * 2 + 6] << 8) | (data[i * 2 + 7] << 0);
    }
    // 响应写多个保持寄存器
    MB_TxBuf[0] = PduData.SlaveAddr;                  // 从机地址
